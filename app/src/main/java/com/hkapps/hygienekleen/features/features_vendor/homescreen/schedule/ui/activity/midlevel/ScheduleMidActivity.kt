package com.hkapps.hygienekleen.features.features_vendor.homescreen.schedule.ui.activity.midlevel
...
import com.hkapps.hygienekleen.features.features_vendor.homescreen.schedule.model.midlevel.MidDataArrayContent
import com.hkapps.hygienekleen.features.features_vendor.homescreen.schedule.ui.adapter.midlevel.ScheduleMidAdapter
...
class ScheduleMidActivity : AppCompatActivity(), ScheduleMidAdapter.ScheduleCallback,
    NoInternetConnectionCallback {
    private lateinit var binding: ActivityScheduleNewBinding
    private lateinit var schAdapter: ScheduleMidAdapter
    private lateinit var schResponseModel: MidScheduleResponseModel
    private var page = 0
...
//                Toast.makeText(this, "" + binding.calView.dayOfMonth, Toast.LENGTH_LONG).show()
    ...
    private fun setObserver() {
        schViewModel.getMidSchResponseModel().observe(this, androidx.lifecycle.Observer { it ->
            if (rvSkeleton.isSkeleton()) rvSkeleton.showOriginal()
            binding.swipeRefreshLayout.isRefreshing = false
            if (it.code == 200) {
                if (it.midDataScheduleResponseModel.contentMid.isNotEmpty()) {
                    binding.tvEmpty.visibility = View.GONE
//                    binding.layoutEmptyState.hide()
                    binding.rvSchedule.show()
                    isLastPage = it.midDataScheduleResponseModel.last
                    if (page == 0) {
                        schResponseModel = it
                        schAdapter =
                            ScheduleMidAdapter(
                                this,
                                it.midDataScheduleResponseModel.contentMid as ArrayList<MidDataArrayContent>
                            ).also { it.setListener(this) }
                        binding.rvSchedule.adapter = schAdapter
                    } else {
                        schAdapter.schContent.addAll(it.midDataScheduleResponseModel.contentMid)
                        schAdapter.notifyItemRangeChanged(
                            schAdapter.schContent.size - it.midDataScheduleResponseModel.contentMid.size,
