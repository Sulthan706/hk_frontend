...
import com.hkapps.hygienekleen.databinding.FragmentWaitingComplaintManagementBinding
...
import com.hkapps.hygienekleen.features.features_management.complaint.ui.adapter.ComplaintManagementByStatusAdapter
...
class WaitingComplaintManagementFragment : Fragment(), ComplaintManagementByStatusAdapter.ComplaintByStatusCallBack {

    private lateinit var binding: FragmentWaitingComplaintManagementBinding
    private lateinit var rvAdapter: ComplaintManagementByStatusAdapter
    ...
    override fun onCreateView(
        ...
        binding = FragmentWaitingComplaintManagementBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // set shimmer effect
        binding.shimmerListComplaintManagementWaiting.startShimmerAnimation()
        binding.shimmerListComplaintManagementWaiting.visibility = View.VISIBLE
        binding.rvListComplaintManagementWaiting.visibility = View.GONE

        // set recycler view
        val layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.VERTICAL, false)
        binding.rvListComplaintManagementWaiting.layoutManager = layoutManager
...
//        binding.swipeListComplaintManagementWaiting.setOnRefreshListener(SwipeRefreshLayout.OnRefreshListener {
//            Handler().postDelayed(
//                Runnable {
//                    binding.swipeListComplaintManagementWaiting.isRefreshing = false
...
//            )
//        })
        binding.rvListComplaintManagementWaiting.addOnScrollListener(scrollListener)
        loadData(combinedArrayList)
        setObserver()
            ...
            if (isLoading != null) {
                if (isLoading) {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed(Runnable {
                        binding.shimmerListComplaintManagementWaiting.stopShimmerAnimation()
                        binding.shimmerListComplaintManagementWaiting.visibility = View.GONE
                        binding.rvListComplaintManagementWaiting.visibility = View.VISIBLE
                    }, 1500)
                }
            }
        })
        viewModel.getListComplaintManagementResponse().observe(requireActivity()) {
            if (it.code == 200) {
                if (it.data.content.isNotEmpty()) {
                    binding.llEmptyListComplaintManagementWaiting.visibility = View.GONE
                    isLastPage = it.data.last
                    if (page == 0) {
                        // set rv adapter
                        rvAdapter = ComplaintManagementByStatusAdapter(
                            requireContext(),
                            it.data.content as ArrayList<Content>,
                            status
                        )
                        binding.rvListComplaintManagementWaiting.adapter = rvAdapter
                    } else {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed(Runnable {
                        binding.llEmptyListComplaintManagementWaiting.visibility = View.VISIBLE
                    }, 1500)
                }
            } else {
