...
import com.hkapps.hygienekleen.databinding.FragmentDoneComplaintVisitorClientBinding
...
import com.hkapps.hygienekleen.features.features_client.complaint.ui.adapter.visitor.DoneComplaintVisitorClientAdapter
...
class DoneComplaintVisitorClientFragment : Fragment(), DoneComplaintVisitorClientAdapter.CtalkDoneVisitorClientCallBack {
    private lateinit var binding: FragmentDoneComplaintVisitorClientBinding
    ...
    private lateinit var adapter: DoneComplaintVisitorClientAdapter
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = FragmentDoneComplaintVisitorClientBinding.inflate(layoutInflater)
    }
...
// set shimmer effect
        ...
        binding.rvHistoryComplaintDoneClient.visibility = View.GONE

        // set recycler view
        val layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.VERTICAL, false)
        binding.rvHistoryComplaintDoneClient.layoutManager = layoutManager
        ...
        binding.rvHistoryComplaintDoneClient.addOnScrollListener(scrollListener)
    ...
    private fun loadData() {
        viewModel.getListCtalkVisitorClient(page, projectId, filter = "DONE")
    }
        ...
        viewModel.getListCtalkVisitorClientViewModel().observe(viewLifecycleOwner){
            if (it.code == 200){
                if (it.data.content.isNotEmpty()){
                    ...
                    binding.rvHistoryComplaintDoneClient.visibility = View.VISIBLE
                    isLastPage = it.data.last
                    if (page == 0){
                        adapter = DoneComplaintVisitorClientAdapter(
                            requireContext(),
                            it.data.content as ArrayList<ContentCtalkVisitorClient>
                        ).also { it.setListener(this) }
                        binding.rvHistoryComplaintDoneClient.adapter = adapter
                    } else {
    ...
    override fun onClickDoneComplaintVisitor(complaintId: Int) {
