...
import com.hkapps.hygienekleen.databinding.ActivityDetailAllManagamentBinding
...
class DetailAllManagamentActivity : AppCompatActivity() {
    private lateinit var binding: ActivityDetailAllManagamentBinding
    ...
    private var managementId =
        CarefastOperationPref.loadInt(CarefastOperationPrefConst.ID_EMPLOYEE_MANAGEMENT, 0)
...
//                val outputDate = convertDate(itemDate)
        ...
        //name management
        binding.tvNameManagementAllLocation.text = managementName.ifEmpty { "" }
    ...
    private fun setObserver() {
        ...
        viewModel.getDetailManagementShareLocViewModel().observe(this){
            if (it.code == 200){
                ...
                binding.tvDateManagementAllLocation.text = it.data.address.ifEmpty { "Data not found" }
            } else {
                binding.llSelectedManagement.visibility = View.GONE
                binding.tvEmptyLocationShareLoc.visibility = View.VISIBLE
                binding.tvDateManagementAllLocation.text = it.message
                binding.tvAddressAllManagement.text = it.message
            }
            hideLoading()
        }
    }
