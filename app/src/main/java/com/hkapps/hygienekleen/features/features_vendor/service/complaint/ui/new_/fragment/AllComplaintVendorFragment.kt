...
import com.hkapps.hygienekleen.databinding.FragmentAllComplaintVendorBinding
import com.hkapps.hygienekleen.features.features_vendor.service.complaint.model.historyComplaint.ListHistoryComplaint
import com.hkapps.hygienekleen.features.features_vendor.service.complaint.ui.new_.activity.ListComplaintVendorActivity
import com.hkapps.hygienekleen.features.features_vendor.service.complaint.ui.new_.adapter.ListComplaintVendorAdapter
...
class AllComplaintVendorFragment : Fragment(), ListComplaintVendorAdapter.ListComplaintCallBack {

    private lateinit var binding: FragmentAllComplaintVendorBinding
    private lateinit var rvAdapter: ListComplaintVendorAdapter
    ...
    override fun onCreateView(
        ...
        binding = FragmentAllComplaintVendorBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // set shimmer effect
        binding.shimmerListComplaintVendor.startShimmerAnimation()
        binding.shimmerListComplaintVendor.visibility = View.VISIBLE
        binding.rvListComplaintVendor.visibility = View.GONE

        // set recycler view
        val layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.VERTICAL, false)
        binding.rvListComplaintVendor.layoutManager = layoutManager
        ...
        binding.swipeListComplaintVendor.setOnRefreshListener(SwipeRefreshLayout.OnRefreshListener {
            Handler().postDelayed(
                Runnable {
                    binding.swipeListComplaintVendor.isRefreshing = false
        ...
        binding.rvListComplaintVendor.addOnScrollListener(scrollListener)
            ...
            if (isLoading != null) {
                if (isLoading) {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed(Runnable {
                        binding.shimmerListComplaintVendor.stopShimmerAnimation()
                        binding.shimmerListComplaintVendor.visibility = View.GONE
                        binding.rvListComplaintVendor.visibility = View.VISIBLE
                    }, 1500)
                }
            }
        })
        viewModel.clientComplaintHistoryResponseModel.observe(requireActivity()) { it ->
            if (it.code == 200) {
                if (it.data.content.isNotEmpty()) {
                    binding.llEmptyListComplaintVendor.visibility = View.GONE
                    isLastPage = it.data.last
                    if (page == 0) {
                        // set rv adapter
                        rvAdapter = ListComplaintVendorAdapter(
                            requireContext(),
                            it.data.content as ArrayList<ListHistoryComplaint>,
                            ""
                        ). also { it.setListener(this) }
                        binding.rvListComplaintVendor.adapter = rvAdapter
                    } else {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed(Runnable {
                        binding.llEmptyListComplaintVendor.visibility = View.VISIBLE
                    }, 1500)
                }
            } else {
