package com.hkapps.hygienekleen.features.features_vendor.homescreen.home.ui.old.fragment

import android.annotation.SuppressLint
import android.content.Intent
import android.os.Build
import android.os.Bundle
import android.os.Handler
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import androidx.core.content.ContextCompat
import androidx.fragment.app.Fragment
import androidx.lifecycle.Observer
import androidx.lifecycle.ViewModelProviders
import com.bumptech.glide.Glide
import com.bumptech.glide.load.engine.DiskCacheStrategy
import com.bumptech.glide.request.RequestOptions
import com.hkapps.hygienekleen.R
import com.hkapps.hygienekleen.databinding.FragmentProfileBinding
import com.hkapps.hygienekleen.features.features_vendor.homescreen.home.ui.new_.activity.updateprofile.UpdateBankAccountActivity
import com.hkapps.hygienekleen.features.features_vendor.homescreen.home.ui.new_.activity.updateprofile.ListCertificateVaksinActivity
import com.hkapps.hygienekleen.features.features_vendor.homescreen.home.ui.new_.activity.updateprofile.ChangeDocumentActivity
import com.hkapps.hygienekleen.features.features_vendor.homescreen.home.ui.new_.activity.updateprofile.UpdateFamilyNumbActivity
import com.hkapps.hygienekleen.features.features_vendor.profile.ui.activity.EditProfileActivity
import com.hkapps.hygienekleen.features.features_vendor.profile.ui.activity.SettingsActivity
import com.hkapps.hygienekleen.features.features_vendor.profile.viewmodel.ProfileViewModel
import com.hkapps.hygienekleen.pref.CarefastOperationPref
import com.hkapps.hygienekleen.pref.CarefastOperationPrefConst


class ProfileFragment : Fragment() {

    lateinit var binding: FragmentProfileBinding
    private val profileViewModel: ProfileViewModel by lazy {
        ViewModelProviders.of(requireActivity()).get(ProfileViewModel::class.java)
    }
    private val employeeId =
        CarefastOperationPref.loadInt(CarefastOperationPrefConst.USER_ID, 0)
    private val employeeName =
        CarefastOperationPref.loadString(CarefastOperationPrefConst.USER_NAME, "")
    private val employeeNuc =
        CarefastOperationPref.loadString(CarefastOperationPrefConst.USER_NUC, "")
    private val employeeJob =
        CarefastOperationPref.loadString(CarefastOperationPrefConst.USER_POSITION, "")
    private var numberNik:String = ""
    private var numberBpjs: String =""
    private var numberJamsostek: String = ""
    private var bpjsImage: String = ""
    private var jamsostekImage:String = ""

    //val
    var nameBank: Int = 0

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        // Inflate the layout for this fragment
        binding = FragmentProfileBinding.inflate(inflater, container, false)


        return binding.root
    }

    @SuppressLint("CheckResult", "SetTextI18n")
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        binding.imgSettings.setOnClickListener {
            val i = Intent(context, SettingsActivity::class.java)
            startActivity(i)
        }

        // finally change the color
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            activity?.window!!.statusBarColor =
                ContextCompat.getColor(requireActivity(), R.color.primary_color)
        }

        binding.shimmerFotoProfile.startShimmerAnimation()

        binding.shimmerNameProfile.startShimmerAnimation()
        binding.shimmerJobProfile.startShimmerAnimation()
        binding.shimmerNucProfile.startShimmerAnimation()
        binding.shimmerProjectNames.startShimmerAnimation()


        Handler().postDelayed(
            Runnable {

                binding.clProfile.visibility = View.VISIBLE
                binding.clShimmer.visibility = View.GONE
                binding.shimmerProjectNames.visibility = View.GONE
                binding.tvProjectNames.visibility = View.VISIBLE

                binding.tvNameUserProfile.text = "" + employeeName
                binding.tvNucProfile.text = "" + employeeNuc
                binding.tvJobUser.text = "" + employeeJob
                //bindig name project


//                val url =
//                    "data:image/jpeg;base64,"
//                context?.let {
//                    Glide.with(it)
//                        .load(url)
//                        .apply(RequestOptions.fitCenterTransform())
//                        .into(binding.ivProfile)
//                }

                profileViewModel.getProfile(employeeId)
                setObserver()

                binding.shimmerFotoProfile.stopShimmerAnimation()
                binding.shimmerNameProfile.stopShimmerAnimation()
                binding.shimmerJobProfile.stopShimmerAnimation()
                binding.shimmerNucProfile.stopShimmerAnimation()
                binding.shimmerProjectNames.stopShimmerAnimation()
            }, 33
        )



    }

    @SuppressLint("SetTextI18n")
    private fun setObserver() {
        profileViewModel.profileResponseModel.observe(viewLifecycleOwner, Observer {

            Handler().postDelayed(
                Runnable {
                    if (it.code == 200) {

                        binding.tvActiveProjectEmployee.text =
                            "Aktif mulai: " + it.data.project.startDate

                        binding.tvCountDocProfile.text = "" + it.data.countDoc
                        binding.tvCountCertiVaksinProfile.text = "" + it.data.countVaccine

//                        validation check
                        if (!it.data.checkBankingAccount) {
                            binding.ivCheckRekeningFalse.visibility = View.VISIBLE
                        } else {
                            binding.ivCheckRekeningTrue.visibility = View.VISIBLE
                            binding.ivCheckRekeningFalse.visibility = View.GONE
                        }
                        if (!it.data.checkProfile) {
                            binding.ivCheckProfileFalse.visibility = View.VISIBLE
                        } else {
                            binding.ivCheckProfileTrue.visibility = View.VISIBLE
                            binding.ivCheckProfileFalse.visibility = View.GONE
                        }
                        if (!it.data.checkFamilyCard) {
                            binding.ivCheckNumbFamilyFalse.visibility = View.VISIBLE
                        } else {
                            binding.ivCheckNumbFamilyTrue.visibility = View.VISIBLE
                            binding.ivCheckNumbFamilyFalse.visibility = View.GONE
                        }

                        if (!it.data.checkImageKtp){
                            binding.ivCheckDokumenKtpFalse.visibility = View.VISIBLE
                        } else {
                            binding.ivCheckDokumenKtpTrue.visibility = View.VISIBLE
                            binding.ivCheckDokumenKtpFalse.visibility = View.GONE
                        }

                        //validation warning all checklist
                        if (!it.data.checkProfile || !it.data.checkFamilyCard || !it.data.checkBankingAccount || !it.data.checkImageKtp) {
                            binding.tvCompletedProfile.visibility = View.VISIBLE
                        } else {
                            binding.tvCompletedProfile.visibility = View.GONE
                        }



                        binding.rlBtnProfileMain.setOnClickListener {
                            startActivity(Intent(context, EditProfileActivity::class.java))
                        }

                        binding.rlBtnProfileRekening.setOnClickListener { _ ->
                            if (!it.data.checkBankingAccount){
                                CarefastOperationPref.saveBoolean(CarefastOperationPrefConst.STATS_BANK_NUMB, false)
                            } else {
                                CarefastOperationPref.saveBoolean(CarefastOperationPrefConst.STATS_BANK_NUMB, true)
                            }
                            startActivity(Intent(context, UpdateBankAccountActivity::class.java))

                            CarefastOperationPref.saveInt(
                                CarefastOperationPrefConst.USER_ID,
                                employeeId
                            )



                            if (!it.data.checkBankingAccount) {
                                nameBank = 0
                                CarefastOperationPref.saveInt(CarefastOperationPrefConst.VALUE_BANK_NAME,
                                0)
                                CarefastOperationPref.saveString(CarefastOperationPrefConst.NUMBER_ACCOUNT,
                                "")
                                CarefastOperationPref.saveBoolean(CarefastOperationPrefConst.STATS_ACCOUNT_PROFILE,
                                it.data.checkBankingAccount)


                            } else {
                                when (it.data.bankingName) {
                                    null -> {
                                        nameBank = 0
                                    }
                                    "Pilih nama bank" -> {
                                        nameBank = 0
                                    }
                                    "BCA" -> {
                                        nameBank = 1
                                    }
                                    "BNI" -> {
                                        nameBank = 2
                                    }
                                    "BRI" -> {
                                        nameBank = 3
                                    }
                                    "DANAMON" -> {
                                        nameBank = 4
                                    }
                                    "MANDIRI" -> {
                                        nameBank = 5
                                    }
                                    "UOB" -> {
                                        nameBank = 6
                                    }
                                }
                                CarefastOperationPref.saveInt(
                                    CarefastOperationPrefConst.VALUE_BANK_NAME,
                                    nameBank
                                )
                                CarefastOperationPref.saveString(
                                    CarefastOperationPrefConst.NUMBER_ACCOUNT,
                                    it.data.bankingNumber
                                )
                                CarefastOperationPref.saveBoolean(
                                    CarefastOperationPrefConst.STATS_ACCOUNT_PROFILE,
                                    it.data.checkBankingAccount
                                )
                            }

                        }

                        binding.rlBtnProfileNomerKK.setOnClickListener { _ ->
                            CarefastOperationPref.saveInt(
                                CarefastOperationPrefConst.USER_ID,
                                employeeId
                            )
                            startActivity(Intent(context, UpdateFamilyNumbActivity::class.java))
                            if (!it.data.checkFamilyCard) {
                                CarefastOperationPref.saveString(
                                    CarefastOperationPrefConst.NUMBERKK,
                                    ""
                                )
                                CarefastOperationPref.saveBoolean(
                                    CarefastOperationPrefConst.STATS_NUMBER_FAMILY,
                                    it.data.checkFamilyCard
                                )
                            } else {
                                CarefastOperationPref.saveBoolean(
                                    CarefastOperationPrefConst.STATS_NUMBER_FAMILY,
                                    it.data.checkFamilyCard
                                )
                                CarefastOperationPref.saveString(
                                    CarefastOperationPrefConst.NUMBERKK,
                                    it.data.familyCardNumber
                                )

                            }
                        }
                        numberNik = it.data.employeeNik

                        numberBpjs = it.data.employeeBpjsKesehatan
                        bpjsImage = it.data.employeeBpjsKesehatanFile

                        numberJamsostek = it.data.employeeBpjsKetenagakerjaan
                        jamsostekImage = it.data.employeeBpjsKetenagakerjaanFile

                        binding.rlBtnProfileDokumen.setOnClickListener {
                            //number bpjs
                            if (numberBpjs == null){
                                CarefastOperationPref.saveString(CarefastOperationPrefConst.NUMBER_BPJS,"")
                            } else {
                                CarefastOperationPref.saveString(CarefastOperationPrefConst.NUMBER_BPJS,numberBpjs)
                            }
                            if (bpjsImage == null){
                                CarefastOperationPref.saveString(CarefastOperationPrefConst.IMAGE_BPJS,"")
                            } else {
                                CarefastOperationPref.saveString(CarefastOperationPrefConst.IMAGE_BPJS, bpjsImage)
                            }

                            if (numberNik == null){
                                CarefastOperationPref.saveString(CarefastOperationPrefConst.NUMBER_NIK,"Number not found")
                            } else {
                                CarefastOperationPref.saveString(CarefastOperationPrefConst.NUMBER_NIK,numberNik)
                            }
                            startActivity(Intent(context, ChangeDocumentActivity::class.java))
                        }
                        binding.rlBtnProfileSertifikatVaksin.setOnClickListener {
                            startActivity(
                                Intent(
                                    context,
                                    ListCertificateVaksinActivity::class.java
                                )
                            )
                        }



                        binding.tvProjectNames.text =
                            "" + it.data.project.projectName
                        loadProfileDefault(it.data.employeePhotoProfile)
                    }

                }, 33
            )

        })
    }

    private fun loadProfileDefault(img: String) {
        val url =
            getString(R.string.url) + "assets.admin_master/images/photo_profile/$img"
        if (img == "null" || img == null || img == "") {
            val uri =
                "@drawable/profile_default" // where myresource (without the extension) is the file
            val imageResource = resources.getIdentifier(uri, null, requireActivity().packageName)
            val res = resources.getDrawable(imageResource)
            binding.ivProfile.setImageDrawable(res)
        } else {
            val requestOptions = RequestOptions()
                .diskCacheStrategy(DiskCacheStrategy.NONE) // because file name is always same
                .skipMemoryCache(true)

            Glide.with(requireActivity())
                .load(url)
                .apply(requestOptions)
                .into(binding.ivProfile)
        }
    }

    override fun onResume() {
        super.onResume()
        profileViewModel.getProfile(employeeId)
    }

}