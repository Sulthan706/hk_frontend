...
class DayTeamSpvFragment: Fragment(), ListLeaderAdapter.ListLeaderCallback {
            ...
            if (isLoading != null) {
                if (isLoading) {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed(Runnable {
                        binding.shimmerShiftSiang.stopShimmerAnimation()
                        binding.shimmerShiftSiang.visibility = View.GONE
                        binding.rvAnggotaShiftSiang.visibility = View.VISIBLE
                    }, 1500)
                }
            }
        ...
        viewModel.listLeaderNewModel.observe(requireActivity()) {
            if (it.code == 200) {
                if (it.data.isNotEmpty()) {
                    ...
                    binding.rvAnggotaShiftSiang.adapter = adapter
                } else {
                    binding.shimmerShiftSiang.visibility = View.GONE
                    binding.rvAnggotaShiftSiang.visibility = View.GONE
                    binding.flShiftSiangNoData.visibility = View.VISIBLE
//                    Toast.makeText(context, "Tidak ada data", Toast.LENGTH_SHORT).show()
    ...
    override fun onClickLeader(leaderId: Int, leaderName: String, shiftId: Int, projectId: String) {
        ...
        i.putExtra("ProjectIdTlDay", projectId)
        startActivity(i)
    }
}