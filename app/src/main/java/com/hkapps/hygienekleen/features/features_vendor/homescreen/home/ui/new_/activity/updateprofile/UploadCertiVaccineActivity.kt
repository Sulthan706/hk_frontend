...
import com.hkapps.hygienekleen.databinding.ActivityUploadCertiVaccineBinding
...
class UploadCertiVaccineActivity : AppCompatActivity(){

    private lateinit var binding: ActivityUploadCertiVaccineBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityUploadCertiVaccineBinding.inflate(layoutInflater)
    ...
    private fun loadDataType(){
        viewModel.getListTypeVaccine(employeeId)
    }
        ...
        if (isGranted) {
            ...
            val resultLauncher = registerForActivityResult(openDocumentContract) { uri ->
                uri?.let {
                    ...
                    val intent = Intent(applicationContext, ListCertificateVaksinActivity::class.java).apply {
    ...
    private fun setObserver() {
        viewModel.updateVaccineViewModel().observe(this){
            if (it.code == 200){
                dialogSuccess()
                finish()
            } else {
        ...
        viewModel.getListTypeVaccineViewModel().observe(this){
            if (it.code == 200){
...
//                val commaSeperatedString = item.joinToString (separator = ", ") { it -> "\'${it.vaccineType}\'" }
                ...
                binding.AcomFieldAdapterVaccine.onItemClickListener = AdapterView.OnItemClickListener { _, _, position, _ ->
                    vaccineType = adapter.getItem(position).toString()
                    // Perform your validation here
                    if (position == AdapterView.INVALID_POSITION) {
                        // No item selected from the adapter
                    } else {
                        // Item selected from the adapter
                        // Proceed with your logic
                        binding.btnUploadDocument.visibility = View.VISIBLE
                        binding.btnUploadDocumentDisable.visibility = View.GONE
                        binding.menu.helperText = null
                    }
                }
                binding.btnUploadDocumentDisable.setOnClickListener {
                    binding.menu.helperText = "isi data"
                }
            } else {
...
//            viewModel.updateVaccine(employeeId, image, typeDocument)
        viewModel.updateVaccine(employeeId, image,vaccineType )
    }
