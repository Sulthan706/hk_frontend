...
import com.hkapps.hygienekleen.databinding.ActivityLivenessBinding
...
class LivenessActivity : AppCompatActivity() {
    ...
    private  var detector : LivenessDetector = LivenessDetector()
    ...
    private fun startCamera() {
        cameraController = LifecycleCameraController(this@LivenessActivity)
...
//                    binding.tvGesture.text = randomizedTasks[currentTaskIndex]
    ...
    private fun finishForResult() {
        ...
        if(!isDone){
           if(isManagement){
               if(isRegister){
                   startActivity(Intent(this@LivenessActivity,ConfirmFacePhotoManagementActivity::class.java).also{
                       it.putExtra("image",result)
                       it.putExtra("images",results)
                   })
                   finish()
               }else{
                   startActivity(Intent(this@LivenessActivity,ResultFacePhotoActivity::class.java).also{
                       it.putExtra("image",result)
                       it.putExtra("images",results)
               ...
               if(isRegister){
                   startActivity(Intent(this@LivenessActivity,ConfirmFacePhotoActivity::class.java).also{
                       it.putExtra("image",result)
                       it.putExtra("images",results)
                   })
                   finish()
    ...
    private fun getGestured(){
        ...
        faceRecognitionViewModel.listGesture.observe(this@LivenessActivity) {
            if (it != null && it.code == 200) {
                detector.listTaskDetection.clear()
                detector.listTaskDetection.addAll(it.data)
                detector.resetTasks()
            } else {
                Toast.makeText(this@LivenessActivity, "Gagal terhubung ke server", Toast.LENGTH_SHORT).show()
            }
        }
    }
