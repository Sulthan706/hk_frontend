...
import com.hkapps.hygienekleen.databinding.FragmentOngoingComplaintVisitorClientBinding
...
import com.hkapps.hygienekleen.features.features_client.complaint.ui.adapter.visitor.OngoingComplaintVisitorClientAdapter
...
class OngoingComplaintVisitorClientFragment : Fragment(), OngoingComplaintVisitorClientAdapter.CtalkOngoingVisitorCallBack {
    private lateinit var binding: FragmentOngoingComplaintVisitorClientBinding
    ...
    private lateinit var adapter: OngoingComplaintVisitorClientAdapter
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
       binding = FragmentOngoingComplaintVisitorClientBinding.inflate(layoutInflater)
    }
...
// set shimmer effect
        ...
        binding.rvHistoryComplaintOngoingClient.visibility = View.GONE

        // set recycler view
        val layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.VERTICAL, false)
        binding.rvHistoryComplaintOngoingClient.layoutManager = layoutManager
        ...
        binding.rvHistoryComplaintOngoingClient.addOnScrollListener(scrollListener)
    ...
    private fun loadData() {
        viewModel.getListCtalkVisitorClient(page, projectId, filter = "ON PROGRESS")
    }
        ...
        viewModel.getListCtalkVisitorClientViewModel().observe(viewLifecycleOwner){
            if (it.code == 200){
                if (it.data.content.isNotEmpty()){
                    ...
                    binding.rvHistoryComplaintOngoingClient.visibility = View.VISIBLE
                    isLastPage = it.data.last
                    if (page == 0){
                        adapter = OngoingComplaintVisitorClientAdapter(
                            requireContext(),
                            it.data.content as ArrayList<ContentCtalkVisitorClient>
                        ).also { it.setListener(this) }
                        binding.rvHistoryComplaintOngoingClient.adapter = adapter
                    } else {
    ...
    override fun onClickOngoingComplaintVisitor(complaintId: Int) {
