...
import com.hkapps.hygienekleen.databinding.FragmentGenerateFileBinding
...
class GenerateFileFragment : Fragment() {

    private lateinit var binding : FragmentGenerateFileBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentGenerateFileBinding.inflate(layoutInflater)
        return binding.root
    }
...
//                    generateFileSPV()
    ...
    private fun generateFile(){
        ...
        if(isGenerated){
            ...
            closingViewModel.generateFileClosing(projectCode,getYesterdayDate(),userId)
            closingViewModel.generateFileClosingModel.observe(viewLifecycleOwner){
    ...
    private fun generateFileSPV(){
        ...
        if(isGenerated){
            ...
            closingViewModel.generateFileClosingSPV(projectCode,getYesterdayDate(),userId)
            closingViewModel.generateFileClosingSPVModel.observe(viewLifecycleOwner){
                if(it.code == 200 || isGenerated){
                    loading?.visibility = View.GONE
                    viewPager?.currentItem = 1
                }else{
    ...
    private fun fetchData() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val urlString = "https://ops.hk-indonesia.id/hksite/Detailperiodic/dataDetailPeriodikDailyTest/$projectCode/${getYesterdayDate()}/$closingId"
