...
import com.hkapps.hygienekleen.databinding.FragmentAllComplaintVisitorClientBinding
...
import com.hkapps.hygienekleen.features.features_client.complaint.ui.adapter.visitor.AllComplaintVisitorClientAdapter
...
class AllComplaintVisitorClientFragment : Fragment(), AllComplaintVisitorClientAdapter.CtalkVisitorClientCallBack {
    private lateinit var binding: FragmentAllComplaintVisitorClientBinding
    ...
    private lateinit var adapter : AllComplaintVisitorClientAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = FragmentAllComplaintVisitorClientBinding.inflate(layoutInflater)
    }
    ...
    private fun loadData() {
        viewModel.getListCtalkVisitorClient(page, projectId, filter = "")
    }
        ...
        viewModel.getListCtalkVisitorClientViewModel().observe(viewLifecycleOwner){
            if (it.code == 200){
                if (it.data.content.isNotEmpty()){
                    ...
                    if (page == 0){
                        adapter = AllComplaintVisitorClientAdapter(
                            requireContext(),
                            it.data.content as ArrayList<ContentCtalkVisitorClient>
                        ).also { it.setListener(this) }
                        binding.rvHistoryComplaint.adapter = adapter
                    } else {
    ...
    override fun onClickComplaintVisitor(complaintId: Int) {
