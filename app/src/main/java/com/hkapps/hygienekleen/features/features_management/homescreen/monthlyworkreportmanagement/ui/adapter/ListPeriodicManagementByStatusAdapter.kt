...
import com.hkapps.hygienekleen.features.features_management.homescreen.monthlyworkreportmanagement.ui.activity.PeriodicManagementByStatusActivity

class ListPeriodicManagementByStatusAdapter(var listByStatsPeriodic: ArrayList<ContentListByStatsPeriodic>):
RecyclerView.Adapter<ListPeriodicManagementByStatusAdapter.ViewHolder>(){

    private lateinit var listClickByStats: PeriodicManagementByStatusActivity
    var selectedItem = -1
    ...
    inner class ViewHolder(val binding: ItemMonthlyWorkreportCardBinding):
        ...
        override fun onClick(p0: View?) {
            val itemByStatsPeriodicMgmnt = listByStatsPeriodic[adapterPosition]
            listClickByStats.onClickByStats(
                itemByStatsPeriodicMgmnt.idJob
            )
        }
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val item = listByStatsPeriodic[position]
        when(item.typeJob){
            "DAILY" -> {
    ...
    override fun getItemCount(): Int {
        return listByStatsPeriodic.size
    }

    fun setListener(periodicManagementByStatusActivity: PeriodicManagementByStatusActivity){
        this.listClickByStats = periodicManagementByStatusActivity
    }

    interface ClickByStatsPeriodic {
        fun onClickByStats(idJob: Int)
    }
