...
import com.hkapps.hygienekleen.databinding.ActivityMonthlyWorkReportCalenderBinding
...
import com.hkapps.hygienekleen.features.features_vendor.homescreen.monthlyworkreport.ui.adapter.ListCalenderMonthlyWorkReportAdapter
...
class PeriodicVendorCalendarActivity : AppCompatActivity(), ListCalenderMonthlyWorkReportAdapter.ClickDateRkb {
    private lateinit var binding : ActivityMonthlyWorkReportCalenderBinding
    ...
    private lateinit var adapter : ListCalenderMonthlyWorkReportAdapter
    private var userId =
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityMonthlyWorkReportCalenderBinding.inflate(layoutInflater)
...
//                val outputDate = convertDate(itemDate)
        ...
        adapter = ListCalenderMonthlyWorkReportAdapter(ArrayList())
    ...
    private fun refreshRecyclerView(data: List<ContentListDateRkb>) {
        adapter.listDateRkb.clear()
        adapter.listDateRkb.addAll(data)
        page = 0
        ...
        viewModelMonthlyWork.getTargetByDateMonthRkbViewModel().observe(this){
            if (it.code == 200){
                ...
                if (it.data.listJobs.content.isNotEmpty()){
                    ...
                    if (page == 0) {
                        adapter = ListCalenderMonthlyWorkReportAdapter(
                            it.data.listJobs.content as ArrayList<ContentListDateRkb>
                        ).also { it.setListener(this) }
                        binding.rvCalenderRkb.adapter = adapter
                    } else {
                        adapter.listDateRkb.addAll(it.data.listJobs.content as ArrayList<ContentListDateRkb>)
                        adapter.notifyItemRangeChanged(
                            adapter.listDateRkb.size - it.data.listJobs.content.size,
                            adapter.listDateRkb.size
                        )
                    }
