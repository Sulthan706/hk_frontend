...
import com.hkapps.hygienekleen.databinding.ActivitySearchOperationalBinding
...
class SearchOperationalActivity : AppCompatActivity(), ListAllOperationalAdapter.ListAllOperationalCallback, ListOperationalProjectAdapter.ListOperationalProjectCallback {

    private lateinit var binding: ActivitySearchOperationalBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.appbarSearchOperationalManagement.svAppbarSearch.queryHint = "Search Operational"
...
//                binding.rvSearchOperationalManagement.visibility = View.GONE
    ...
    private fun setObserver() {
        ...
        viewModel.getSearchOperationalCeoBodResponse().observe(this) {
            if (it.code == 200) {
...
//                            this
        ...
        viewModel.getSearchOperationalGmOmFmResponse().observe(this){
            if(it.code == 200){
                if(it.data.content.isNotEmpty()){
                    binding.flNoDataSearchOperationalManagement.visibility = View.GONE
                    isLastPage = it.data.last
    ...
    private fun loadData(query: String) {
        when(userPosition){
            "CEO", "BOD" -> {
                viewModel.getSearchOperationalCeoBod(page, query)
            }
            "GM", "OM", "FM", "CLIENT" -> {
                viewModel.getSearchOperationalGmOmFm(employeeId, page, query)
            }
        }
    ...
    override fun onClickOps(idEmployee: Int, projectCode: String) {
        ...
        CarefastOperationPref.saveString(
            CarefastOperationPrefConst.OPERATIONAL_OPS_PROJECT_CODE,
            projectCode
        )
    }
