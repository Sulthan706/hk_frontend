...
class AttendanceOutManagementActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (image != null) {
            ...
            var flag = 1
            binding.btnAttendancePhotoManagement.text = "Absen Keluar"
...
//                    }
    ...
    private fun setObserver(image : ArrayList<String>, bitmap: Bitmap) {
        viewModel.attendanceCheckOutGeoModel.observe(this) {
            if (it.code == 200) {
                hideLoading()
...
//                startActivity(Intent(this, StatsVerifyManagementActivity::class.java))
        ...
        viewModel.attendanceOutGeoV2Model.observe(this) {
            if (it.code == 200) {
                hideLoading()
                val date = Calendar.getInstance()
                val day = android.text.format.DateFormat.format("dd", date) as String
...
//                Toast.makeText(this, "Gagal melakukan absen masuk", Toast.LENGTH_SHORT).show()
        ...
        viewModel.attendanceOutGeoBodV2Response.observe(this) {
            if (it.code == 200) {
                hideLoading()
                val date = Calendar.getInstance()
                val day = android.text.format.DateFormat.format("dd", date) as String
...
//                    finish()
//                }
            resultObserver(it.message,it.status,it.errorCode ?: "",image)
        }
    ...
    private fun resultObserver(message : String,status : String,errorCode : String,image : ArrayList<String>){
        if (status == "SUCCESS"){
            attendance("Loading..")
        }
...
//        viewModel.attendanceCheckInGeo(userId, projectCode, jabatan, image)
    ...
    private fun attendance(loadingText: String){
        ...
        // load api check out attendance
        capturedImageUri = Uri.fromFile(file)
        if (userLevel == "BOD" || userLevel == "CEO" || isVp) {
            viewModel.attendanceOutGeoBodV2(userId, idRkb, image, longitudes, latitudes, addressApi)
        } else {
            when (featureAccess) {
                "V1" -> viewModel.attendanceOutGeo(userId, projectCode, image, longitudes, latitudes, addressApi)
                "V2" -> viewModel.attendanceOutGeoV2(userId, idRkb, image, longitudes, latitudes, addressApi)
                else -> Toast.makeText(this, "Can't hit api attendance out", Toast.LENGTH_SHORT).show()
            }
        }
