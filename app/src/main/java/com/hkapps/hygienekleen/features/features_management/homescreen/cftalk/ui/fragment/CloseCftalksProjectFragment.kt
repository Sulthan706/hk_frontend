...
import com.hkapps.hygienekleen.databinding.FragmentCloseCftalksProjectBinding
...
class CloseCftalksProjectFragment : Fragment(),
    StatusCftalksProjectAdapter.CftalkManagementCallBack {

    private lateinit var binding: FragmentCloseCftalksProjectBinding
...
//    private val projectId = "CFHO"
    ...
    override fun onCreateView(
        ...
        binding = FragmentCloseCftalksProjectBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // set shimmer effect
        binding.shimmerCloseCftalkProject.startShimmerAnimation()
        binding.shimmerCloseCftalkProject.visibility = View.VISIBLE
        binding.rvCloseCftalkProject.visibility = View.GONE

        // set recycler view
        val layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.VERTICAL, false)
        binding.rvCloseCftalkProject.layoutManager = layoutManager
        ...
        binding.swipeCloseCftalkProject.setOnRefreshListener(SwipeRefreshLayout.OnRefreshListener {
            Handler().postDelayed(
                Runnable {
                    binding.swipeCloseCftalkProject.isRefreshing = false
        ...
        binding.rvCloseCftalkProject.addOnScrollListener(scrollListener)
            ...
            if (isLoading != null) {
                if (isLoading) {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed(Runnable {
                        binding.shimmerCloseCftalkProject.stopShimmerAnimation()
                        binding.shimmerCloseCftalkProject.visibility = View.GONE
                        binding.rvCloseCftalkProject.visibility = View.VISIBLE
                    }, 1500)
                }
            }
        })
        viewModel.complaintsByProjectModel.observe(requireActivity()) {
            if (it.code == 200) {
                if (it.data.content.isNotEmpty()) {
                    binding.tvCloseCftalkProject.visibility = View.GONE
                    ...
                    if (page == 0) {
                        ...
                        binding.rvCloseCftalkProject.adapter = rvAdapter
                    } else {
                    ...
                    binding.rvCloseCftalkProject.adapter = null
                    Handler(Looper.getMainLooper()).postDelayed(Runnable {
                        binding.tvCloseCftalkProject.visibility = View.VISIBLE
                    }, 1500)
                }
            } else {
