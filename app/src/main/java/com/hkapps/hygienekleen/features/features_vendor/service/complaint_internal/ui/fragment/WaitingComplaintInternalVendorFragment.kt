...
import com.hkapps.hygienekleen.databinding.FragmentWaitingComplaintInternalVendorBinding
...
class WaitingComplaintInternalVendorFragment : Fragment(), ComplaintInternalByStatusVendorAdapter.ComplaintStatusCallback {
    private lateinit var binding: FragmentWaitingComplaintInternalVendorBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentWaitingComplaintInternalVendorBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // set shimmer effect
        binding.shimmerListComplaintVendorWaiting.startShimmerAnimation()
        binding.shimmerListComplaintVendorWaiting.visibility = View.VISIBLE
        binding.rvListComplaintVendorWaiting.visibility = View.GONE

        // set recycler view
        val layoutManager = LinearLayoutManager(requireContext(), LinearLayoutManager.VERTICAL, false)
        binding.rvListComplaintVendorWaiting.layoutManager = layoutManager
        ...
        binding.swipeListComplaintVendorWaiting.setOnRefreshListener(SwipeRefreshLayout.OnRefreshListener {
            Handler().postDelayed(
                Runnable {
                    binding.swipeListComplaintVendorWaiting.isRefreshing = false
        ...
        binding.rvListComplaintVendorWaiting.addOnScrollListener(scrollListener)
            ...
            if (isLoading != null) {
                if (isLoading) {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed(Runnable {
                        binding.shimmerListComplaintVendorWaiting.stopShimmerAnimation()
                        binding.shimmerListComplaintVendorWaiting.visibility = View.GONE
                        binding.rvListComplaintVendorWaiting.visibility = View.VISIBLE
                    }, 1500)
                }
            }
        })
        viewModel.complaintObs().observe(requireActivity()){
            if(it.code == 200){
                if(it.data.content.isNotEmpty()){
                    binding.llEmptyListComplaintVendorWaiting.visibility = View.GONE
                    ...
                    if(page == 0){
                        ...
                        binding.rvListComplaintVendorWaiting.adapter = rvAdapter
                    }else {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed(Runnable {
                        binding.llEmptyListComplaintVendorWaiting.visibility = View.VISIBLE
                    }, 1500)
                }
            }else {
