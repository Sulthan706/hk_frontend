...
import com.hkapps.hygienekleen.databinding.ListEmployeeHistoryAttendanceProjectBinding
import com.hkapps.hygienekleen.features.features_client.dashboardProject.model.listEmployee.Pengawas

class TeamPengawasProjectAdapter(
    private val context: Context,
    var listEmployee: ArrayList<Pengawas>):
    RecyclerView.Adapter<TeamPengawasProjectAdapter.ViewHolder>() {

    inner class ViewHolder(val binding: ListEmployeeHistoryAttendanceProjectBinding):
        RecyclerView.ViewHolder(binding.root) {

    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return ViewHolder(ListEmployeeHistoryAttendanceProjectBinding.inflate(LayoutInflater.from(parent.context), parent, false))
    }

    @SuppressLint("UseCompatLoadingForDrawables")
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val response = listEmployee[position]
        ...
        when (response.statusAttendance) {
            ...
            "TIDAK_HADIR", "Tidak_hadri", "Tidak_Hadir" -> {
                holder.binding.tvStatusAbsentEmployeeHistoryAttendanceProject.setTextColor(context.resources.getColor(R.color.red1))
            }
            "SEDANG BEKERJA", "Sedang Bekerja", "Sedang bekerja", "Bertugas", "BERTUGAS",
            "SEDANG_BEKERJA", "Sedang_bekerja", "Sedang_Bekerja" -> {
                holder.binding.tvStatusAbsentEmployeeHistoryAttendanceProject.setTextColor(context.resources.getColor(R.color.blue))
            }
            "SELESAI", "Selesai", "HADIR", "Hadir" -> {
                holder.binding.tvStatusAbsentEmployeeHistoryAttendanceProject.setTextColor(context.resources.getColor(R.color.green))
            }
            "LIBUR", "Libur" -> {
                holder.binding.tvStatusAbsentEmployeeHistoryAttendanceProject.setTextColor(context.resources.getColor(R.color.grey2_client))
            }
        }
